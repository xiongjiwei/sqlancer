...
import static sqlancer.tidb.TiDBSchema.TiDBDataType.JSON;
...
public class TiDBExpressionGenerator extends UntypedExpressionGenerator<TiDBExpression, TiDBColumn> {
    ...
    protected TiDBExpression generateExpression(int depth) {
        ...
        switch (Randomly.fromOptions(Gen.values())) {
            ...
            case CAST:
                if (Randomly.getBooleanWithRatherLowProbability() && columns.size() == 1 && columns.get(0).getType().getPrimitiveDataType() == JSON) {
                    return new TiDBCastOperation(generateColumn(),
                            Randomly.fromOptions("BINARY", "CHAR", "DATE", "DATETIME", "TIME", "DECIMAL", "SIGNED", "UNSIGNED"),
                            true
                    );
                }
    ...
    public TiDBExpression generateConstant() {
        return generateConstant(TiDBDataType.getRandom());
    }
    ...
    public TiDBExpression generateConstant(TiDBDataType type) {
        ...
        switch (type) {
            ...
            case NUMERIC:
                return TiDBConstant.createIntConstant(globalState.getRandomly().getInteger());
            case JSON:
                return TiDBConstant.createJSONConstant(globalState.getRandomly().getJSON());
            default:
                throw new AssertionError();
        }
    }
